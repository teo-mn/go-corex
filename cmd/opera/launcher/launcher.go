...
import (
	...
	"github.com/corex-mn/go-corex/cmd/opera/launcher/metrics"
	"github.com/corex-mn/go-corex/cmd/opera/launcher/tracing"
	"github.com/corex-mn/go-corex/debug"
	"github.com/corex-mn/go-corex/flags"
	...
	_ "github.com/corex-mn/go-corex/version"
)

const (
	// clientIdentifier to advertise over the network.
	clientIdentifier = "go-corex"
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-corex command line interface")
	...
	if ctx.GlobalBool(utils.ExitWhenSyncedFlag.Name) {
		go func() {
			for first := true; ; first = false {
				// Call crx_syncing until it returns false
				time.Sleep(5 * time.Second)

				var syncing bool
				err := rpcClient.CallContext(context.TODO(), &syncing, "crx_syncing")
				if err != nil {
					continue
				}
				if !syncing {
