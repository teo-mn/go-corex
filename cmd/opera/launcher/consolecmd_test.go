...
const (
	ipcAPIs  = "abft:1.0 admin:1.0 crx:1.0 dag:1.0 debug:1.0 net:1.0 personal:1.0 rpc:1.0 sfc:1.0 txpool:1.0 web3:1.0"
	httpAPIs = "abft:1.0 crx:1.0 dag:1.0 rpc:1.0 sfc:1.0 web3:1.0"
)
...
instance: go-corex/v{{version}}/{{goos}}-{{goarch}}/{{gover}}
coinbase: {{.Coinbase}}
at block: 1 ({{niltime}})
 datadir: {{.Datadir}}
 modules: {{apis}}
...
func TestIPCAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		...
		ipc = filepath.Join(ws, "corex.ipc")
	}
	cli := exec(t,
		"--fakenet", "0/1", "--port", "0", "--maxpeers", "0", "--nodiscover", "--nat", "none",
		"--ipcpath", ipc)
