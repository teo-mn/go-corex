...
//go:generate bash -c "cd ../../corex-sfc && git checkout 8e3489abeadbb772401fdb200e56e0b84f9f7172"
//go:generate bash -c "docker run --name go-corex-sfc-compiler -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../corex-sfc:/src -w /src node:10.5.0 bash -c 'export NPM_CONFIG_PREFIX=~; npm install --no-save; npm install --no-save truffle@5.1.4' && docker commit go-corex-sfc-compiler go-corex-sfc-compiler-image && docker rm go-corex-sfc-compiler"
//go:generate bash -c "docker run --rm -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../corex-sfc:/src -w /src go-corex-sfc-compiler-image bash -c 'export NPM_CONFIG_PREFIX=~; rm -f /src/build/contracts/*json; npm run build'"
...
//go:generate bash -c "docker run --rm -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../corex-sfc:/src -w /src go-corex-sfc-compiler-image bash -c 'export NPM_CONFIG_PREFIX=~; sed -i s/runs:\\ 200,/runs:\\ 10000,/ /src/truffle-config.js; rm -f /src/build/contracts/*json; npm run build'"
//go:generate bash -c "cd ../../corex-sfc && git checkout -- truffle-config.js; docker rmi go-corex-sfc-compiler-image"
...
import (
	...
	"github.com/corex-mn/go-corex/gossip/contract/driver100"
	"github.com/corex-mn/go-corex/gossip/contract/driverauth100"
	"github.com/corex-mn/go-corex/gossip/contract/netinit100"
	"github.com/corex-mn/go-corex/gossip/contract/sfc100"
	"github.com/corex-mn/go-corex/logger"
	"github.com/corex-mn/go-corex/opera/genesis/driver"
	"github.com/corex-mn/go-corex/opera/genesis/driverauth"
	"github.com/corex-mn/go-corex/opera/genesis/evmwriter"
	"github.com/corex-mn/go-corex/opera/genesis/netinit"
	"github.com/corex-mn/go-corex/opera/genesis/sfc"
...
func TestSFC(t *testing.T) {
	...
	_ = true &&
		...
		t.Run("SFC upgrade", func(t *testing.T) {
			...
			rr := env.ApplyBlock(nextEpoch,
				env.Contract(admin, utils.ToCrx(0), sfc100.ContractBin),
			)
	...
	t.Run("Direct driver", func(t *testing.T) {
		...
		rr := env.ApplyBlock(nextEpoch,
			env.Contract(admin, utils.ToCrx(0), anyContractBin),
		)
		require.Equal(1, rr.Len())
		require.Equal(types.ReceiptStatusSuccessful, rr[0].Status)
		newImpl := rr[0].ContractAddress
...
func cicleTransfers(t *testing.T, env *testEnv, count uint64) {
	...
	for i := uint64(0); i < count; i++ {
		...
		for i := range txs {
			from := (i)%accounts + 1
			to := (i+1)%accounts + 1
			txs[i] = env.Transfer(from, to, utils.ToCrx(100))
		}
